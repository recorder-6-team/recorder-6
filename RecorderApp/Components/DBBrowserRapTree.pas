...
type
  ...
  private
    ...
    FQuery : TJNCCQuery;
    procedure GetRecordStruct(iKeyList: TKeyList; iNode: TFlyNode);
    procedure GetRecordStrings(ioRecordList: TStrings;
      const iTable: string; const iKey1, iKey2: TKeyString);
  public
    constructor Create(iTreeView : TRapidTree; AConnection: TADOConnection);
    destructor Destroy; override;
    procedure SetTreeViewItem(iSourceData: TKeyList);
    procedure PopulateRelationship(iRelIndex: integer; iNode: TFlyNode);
  end;
...
const
  EST_NIL_LIST = 'A call was made to GetRecordStrings with a nil string list';
  EST_RECORD_MISSING = 'The record was not found in table : ';
...
constructor TDBBrowserUtility.Create(iTreeView : TRapidTree;
...
begin
  ...
  FTreeView.Selected := lTableNode;
  lTableNode.Selected := True;
end;
...
begin
  if ioRecordList = nil then
    raise EDBBrowserUtility.Create(EST_NIL_LIST);
  ...
  begin
    ...
    try
      if EOF then
        raise EDBBrowserUtility.Create(EST_RECORD_MISSING + iTable);
