...
uses
  Windows, ExceptionForm, SysUtils, Classes, Math, Dialogs, Recorder2000_TLB,
...
type
  ...
  EOutOfRangeError = class(TExceptionPath);
...
function CheckBoundingBox(
    const iSWSpatialRef, iNESpatialRef, iCurrentSystem: string;
    const iInputSystem: string = ''): TValidBoundingBox;
...
function GetDisplaySpatialRef(
    const iDisplaySystem, iSpatialRef, iSpatialRefSystem: string;
    iLat, iLong: Variant;
    const iFieldString: string): string; overload;
...
{ Converts the given (delocalised) longitude to a floating-point number. }
function LongitudeToFloat(const Longitude: string): Double;
...
{-------------------------------------------------------------------------------
  Takes a delocalised lat-long spatial reference and removes all characters
  that are neither digits nor in ['N', 'E', 'S', 'W', '+', '-', '.'].
------------------------------------------------------------------------------}
function RemoveNonLLSubstrings(SpatialRef: string): string;
var
  I: Integer;
begin
  Result := '';
  for I := 1 to Length(SpatialRef) do
    if SpatialRef[i] in ['0'..'9', 'N', 'E', 'S', 'W', '+', '-', '.'] then
      Result := Result + SpatialRef[I];
end;
...
begin
  ...
  if not assigned(Result) then
    raise ESpatialRefError.CreateNonCritical(ResStr_UnrecognisedSystem);
end;
...
begin
  ...
  begin { check for a COM system }
    ...
    Result := lComSystemIntf.ConvertFromLatLong(
        FloatToStr(iLatLong.Lat, CanonicalFormatSettings),
        FloatToStr(iLatLong.Long, CanonicalFormatSettings));
    if Result = '' then
    ...
    on EIntfCastError do begin
      try
        ...
        on EIntfCastError do
          Raise ESpatialRefError.CreateNonCritical
...
{ Depending on the base map system, the easting northing will represent a
  ...
  if SameText(iSystem, UTM) then
    ESpatialRefError.CreateNonCritical(ResStr_CannotConvertEN)
  else
  if SameText(iSystem, OS_GB) then
...
begin
  ...
  { UTM }
  if SameText(iSystem, UTM) then
    raise ESpatialRefError.CreateNonCritical(ResStr_CannotConvertEN)
  else
  { UK Grid }
  if SameText(iSystem, OS_GB) then
...
{ Return true if the supplied reference can be interpreted as a UTM reference. }
function IsValidUTMRef(const iSpatialRef : string): boolean;
var
  lPos: Integer;
begin
  ...
  lPos := PosEx(' ', iSpatialRef, 1);
  if lPos < 2 then Exit;

  lPos := PosEx(' ', iSpatialRef, lPos + 1);
  if lPos < 4 then Exit;

  if PosEx(' ', iSpatialRef, lPos + 1) <> 0 then Exit;

  Result := ValidInteger(StringReplace(iSpatialRef, ' ', '', [rfReplaceAll]))
      and (Length(iSpatialRef) in [15..17]);
end;
...
begin
  if iSigFigs = -1 then
    ...
    lSigFigs := iSigFigs;
  Result := LatToString(iLatLong.Lat, lSigFigs)
...
{==============================================================================
    Function  Name: ValidUTMSpatialRef
            Called: ValidSpatialRef, ValidSpecificSpatialRef
           Purpose: The passed in spatial reference is checked to ensure that
                     it is composed of three integers separated by spaces.
...
var
  ...
  lCount : integer;
begin
  { UTM should be three integers separated by spaces }
  If not IsValidUTMRef (iSpatialRef) then
  begin
    Result.FormattedSR := ResStr_BeyondSystemLimits;
    Result.Valid := False;
    Result.Error := ResStr_InvalidSpRefFormat;
...
begin
  ...
  If (lResult.x < 0) or (lResult.y < 0) then
    raise ESpatialRefError.CreateNonCritical(ResStr_OutsideSystem);
...
begin
  ...
  if ((iInputSystem = '') or (iInputSystem = ResStr_SystemUnknown))
    and (DetermineSpatialRefSystem(iSWSpatialRef)
  ...
  lneValid := ValidSpatialRef(iNESpatialRef, iCurrentSystem, iInputSystem);
  If lneValid.Valid then
    Result.FormattedNorthEastSR := lneValid.FormattedSR
  else
    Result.Error := lneValid.Error;

  lswValid := ValidSpatialRef(iSWSpatialRef, iCurrentSystem, iInputSystem);
...
function FormatTLatLongAsString(const iLatLong : TLatLong) : String;
begin
  Result := LatToString(iLatLong.Lat, LAT_LONG_SIGFIGS)
      + CanonicalFormatSettings.ListSeparator + ' '
...
begin
  ...
  if (iParentSystem = LAT_LONG) or (iParentSystem = UTM) then
    Raise ESpatialRefError.CreateNoncritical(ResStr_SameSpatialRef);
...
function GetDisplaySpatialRef(
  const iDisplaySystem, iSpatialRef, iSpatialRefSystem: string;
  iLat, iLong: Variant;
  const iFieldString: string): string;
begin
  ...
  begin
    ...
    try
      ...
      if VarIsNumeric(iLat) and VarIsNumeric(iLong) then begin
        if iDisplaySystem <> LAT_LONG then
          Result := ConvertFromLatLong(LatLong(iLat, iLong), iDisplaySystem)
        else
          Result := LatToString(iLat, LAT_LONG_SIGFIGS) +
                    CanonicalFormatSettings.ListSeparator + ' ' +
                    LongToString(iLong, LAT_LONG_SIGFIGS);
      end else
        Result := iFieldString;
    except
      on E:ESpatialRefError do
...
begin
  ...
  begin
    ...
    begin
      ...
      lValidSR := ValidSpatialRef(DelocaliseSpatialRef(iControl.Text), iSystem);
      lValidated := True;
    ...
    begin
      ...
      begin
        lValidSR :=
            ValidSpatialRef(DelocaliseSpatialRef(iControl.Text), iSystem);
        if lValidSR.Valid then
          iEnteredRef := lValidSr.EnteredFormattedSR;
      end
      else
  ...
  begin
    if not lValidSR.Valid then
      If iControl.Modified then
        raise ESpatialRefError.CreateValidation(lValidSr.Error, iControl);
    iControl.Text := LocaliseSpatialRef(lValidSR.FormattedSR);
  end;
end;
...
begin
  ...
  if iSigPlaces <> 0 then
    Result := FloatToStr(
        Abs(RoundTo(idblValue, 0-iSigPlaces)),
        CanonicalFormatSettings)
  else
    Result := FloatToStr(Abs(idblValue), CanonicalFormatSettings);
...
{-------------------------------------------------------------------------------
  Converts a string to a latitude or longitude value.
  
  The string is assumed to be in canonical (delocalised) form; the presence of
  one of the given characters at the end of the string indicates whether the
  value is positive or negative respectively. If neither character is present
  then the value is assumed to be positive.
}
function StringToLatOrLong(const Str: string; PosChar, NegChar: Char): Double;
var
  SignPos: Integer;
begin
  SignPos := Length(Str);
  if SignPos = 0 then
    raise EConvertError.Create(ResStr_InvalidSpRefFormat);
    
  if not (Str[SignPos] in [PosChar, NegChar]) then
    Result := StrToFloat(Str, CanonicalFormatSettings)
  else
  begin
    Result := StrToFloat(LeftStr(Str, SignPos - 1), CanonicalFormatSettings);
    if Str[SignPos] = NegChar then Result := -1 * Result;
  end;
end;
...
function LocaliseSpatialRef(const ASpatialRef: String): String;
begin
  Result := StringReplace(ASpatialRef, '.', #1, [rfReplaceAll]);
  Result := StringReplace(Result, ',', ListSeparator, [rfReplaceAll]);
  Result := StringReplace(Result, #1, DecimalSeparator, [rfReplaceAll]);
end;
...
function DelocaliseSpatialRef(const ASpatialRef: String): String;
begin
  Result := StringReplace(ASpatialRef, DecimalSeparator, #1, [rfReplaceAll]);
  Result := StringReplace(Result, ListSeparator, ',', [rfReplaceAll]);
...
function LatitudeToFloat(const Latitude: string): Double;
begin
  Result := StrToFloat(Latitude, CanonicalFormatSettings);
end;
...
function LongitudeToFloat(const Longitude: string): Double;
begin
  Result := StrToFloat(Longitude, CanonicalFormatSettings);
end;
