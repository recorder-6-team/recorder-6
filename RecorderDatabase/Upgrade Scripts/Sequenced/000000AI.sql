...
ALTER TABLE SURVEY
ADD  LICENCE_KEY char(16) null

GO

ALTER TABLE SURVEY
ADD  PRIVATE_NOTES  Text null

GO

ALTER TABLE SURVEY

ADD  ATTRIBUTION  Text null
...
ALTER TABLE [dbo].[SURVEY]  WITH NOCHECK ADD  CONSTRAINT [FK_SURVEY_LICENCE] FOREIGN KEY([LICENCE_KEY])
REFERENCES [dbo].[LICENCE] ([LICENCE_KEY])

GO

ALTER TABLE [dbo].[SURVEY] CHECK CONSTRAINT [FK_SURVEY_LICENCE]
GO



GRANT SELECT ON [dbo].[LICENCE] TO R2k_AddOnly 
GRANT INSERT ON [dbo].[LICENCE] TO R2k_AddOnly 
GRANT SELECT ON [dbo].[LICENCE] TO R2k_ReadOnly 
GRANT SELECT ON [dbo].[LICENCE] TO R2k_RecordCardsOnly
GRANT SELECT ON [dbo].[LICENCE] TO R2k_Administrator 
GRANT INSERT ON [dbo].[LICENCE] TO R2k_Administrator 
GRANT DELETE ON [dbo].[LICENCE] TO R2k_Administrator 
GRANT UPDATE ON [dbo].[LICENCE] TO R2k_Administrator 
GRANT SELECT ON [dbo].[LICENCE] TO R2k_FullEdit
GRANT INSERT ON [dbo].[LICENCE] TO R2k_FullEdit 
GRANT DELETE ON [dbo].[LICENCE] TO R2k_FullEdit
GRANT UPDATE ON [dbo].[LICENCE] TO R2k_FullEdit
GO


CREATE TRIGGER [dbo].[LICENCECustodianInsert] ON [dbo].[LICENCE] AFTER INSERT AS UPDATE LICENCE SET LICENCE.CUSTODIAN = SUBSTRING(LICENCE.LICENCE_KEY, 1, 8) FROM LICENCE INNER JOIN INSERTED ON LICENCE.LICENCE_KEY = INSERTED.LICENCE_KEY WHERE LICENCE.CUSTODIAN IS NULL
...
'FROM #REPORT_OUTPUT INNER JOIN SURVEY INNER JOIN LICENCE ON SURVEY.LICENCE_KEY = LICENCE.LICENCE_KEY
...
'#REPORT_OUTPUT.[Survey Licence (Long name)] = LICENCE.LONG_NAME',
'NBNSYS0000000201','TESTDATA00000001', Getdate(),1)

INSERT INTO REPORT_ATTRIBUTE (REPORT_ATTRIBUTE_KEY,ITEM_GROUP,SOURCE_TABLE,ITEM_NAME,
ATTRIBUTE_SQL, REPORT_JOIN_KEY,ENTERED_BY,ENTRY_DATE,SYSTEM_SUPPLIED_DATA)
VALUES ('NBNSYS0000000202','Survey','SURVEY', 'Survey Licence (Short name)',
'#REPORT_OUTPUT.[Survey Licence (Short name)] = LICENCE.SHORT_NAME',
'NBNSYS0000000201','TESTDATA00000001', Getdate(),1)
...
DELETE FROM DATABASE_RELATIONSHIP WHERE RELATIONSHIP_KEY = 'SYSTEM0200000009'

INSERT INTO DATABASE_RELATIONSHIP (RELATIONSHIP_KEY,RELATIONSHIP_NAME,
MASTER_TABLE,MASTER_FIELD,DETAIL_TABLE,DETAIL_FIELD,
FOLLOW_UP,FOLLOW_DOWN,ONE_TO_ONE)
VALUES ('SYSTEM0200000009','SURVEY_LICENCES','LICENCE','LICENCE_KEY','SURVEY','LICENCE_KEY',
1,0,0)


GO

INSERT INTO TERM_LIST ([TABLE],KEY_FIELD,DESCRIPTION,SYSTEM_SUPPLIED_DATA,ADDITIONAL_FIELDS)
VALUES  ('LICENCE','LICENCE_KEY','Licences',1,0)
