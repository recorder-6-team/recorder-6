...
type
  ...
  fvOptionEnum = (fvMetaRefresh, fvNoBorder, fvNoLinkUnderline, fvOverLinksActive,
                  fvPrintMonochromeBlack, fvPrintTableBackground, fvShowVScroll);
...
{common base class for TFrameViewer and TFrameBrowser}
  ...
  published
    ...
    property OnLink: TLinkType read FOnLink write SetOnLink;    {8.31}
...
if CurFrameSet.TriggerEvent(Src, @EV) then
  with EV do
    ...
    case LStyle of
      ...
      lsStream:
        UrlRequestStream.LoadFromStream(Stream);
      lsStrings:
        begin
        S := Strings.Text;
        UrlRequestStream.SetSize(Length(S));
        System.Move(S[1], UrlRequestStream.Memory^, Length(S));
        end;
      lsBuffer:
        begin
        UrlRequestStream.SetSize(BuffSize);
        System.Move(Buffer^, UrlRequestStream.Memory^, BuffSize);
        end;
      end;
...
if Assigned(AViewer) and (AViewer.SelLength <> 0) then
  if Abs(AViewer.SelLength) < 32000 then
