...
type
  ...
  BorderStyleType = (bssNone, bssSolid);
...
{$endif}
  ...
  PropIndices = (
      ...
      BorderStyle, Color, BackgroundColor, BorderColor,
...
const                             
  PropWords: array[Low(PropIndices)..High(PropIndices)] of string =
    ('font-family', 'font-size', 'font-style', 'font-weight', 'text-align',
     'text-decoration', 'border-style', 'color', 'background-color', 'border-color',
...
if VarType(Item) = VarString then
  ...
  if I > 0 then
    ...
    // look for the last bracket to close the URL, as the URL can contain brackets itself
    I := Pos(')', reverse);
    I := length(S)-I;
...
for I := Low(I) to High(I) do
  case I of
    MarginTop..LeftPos:
      Props[I] := IntNull;
    BackgroundColor, BorderColor, VerticalAlign, BorderStyle,
...
const
 ...
 { SetValues: array[1..19] of integer =
    (0, 1, 2, 77, 128, 129, 130, 134, 136, 161, 162, 163, 177, 178,
     186, 204, 222, 238, 255); }   
  CodePages: array[1..19] of integer =
...
if VarType(Props[BorderStyle]) = VarString then
  begin
  if Props[BorderStyle] = 'none' then
    Result := bssNone
  else Result := bssSolid;
  end
else Result := bssNone;
end;
  ...
  for Index := Low(Index) to High(PropIndices) do
      if (VarType(Source.Props[Index]) <> varEmpty) and (Vartype(Source.Props[Index]) <> varNull) then
        case Index of
          ...
          FontFamily, FontSize, FontStyle, FontWeight, Color, BackgroundColor, TextDecoration:
            begin
            Originals[Index] := True;
            Props[Index] := Source.Props[Index];
            if InLink then
              for I := LFont to HVFont do
                with FIArray.Ar[I] do
                  case Index of
                    ...
                    FontSize:  iSize := Round(FontSizeConv(Props[FontSize], iSize));
...
if not Assigned(TheFont) then
  ...
  DC := GetDC(0);   {calculate EmSize with current font rather than inherited}
  try
...
Font.iStyle := Style;
Font.iSize := Round(Props[FontSize]);
  ...
  case I of
    BorderTopStyle..BorderLeftStyle:
      if VarType(Props[I]) = VarString then
        begin
        if Props[I] = 'none' then
          MArray[I] := bssNone
        else MArray[I] := bssSolid;
        end
...
with Properties do
  begin
  Props[FontFamily] := 'MS sans serif, sans-serif';
  Props[FontSize] := '10pt';
  Props[FontStyle] := 'none';
  Props[FontWeight] := 'normal';
  Props[TextAlign] := 'left';
