...
Const
  ...
  ResWords : array[1..MaxRes] of SymString =
     ('HTML', 'TITLE', 'BODY', 'HEAD', 'B', 'I', 'H', 'EM', 'STRONG',
      ...
      'S', 'STRIKE', 'TR', 'NOBR', 'STYLE', 'SPAN', 'COLGROUP',
  ...
  EndResSy : array[1..MaxEndRes] of Symb =
     (HtmlEndSy, TitleEndSy, BodyEndSy, HeadEndSy, BEndSy, IEndSy, HeadingEndSy,
      ...
      NoBrEndSy, StyleEndSy, SpanEndSy, ColGroupEndSy);
...
if (Integer(Buff) and $FFF = 0)    {about every 4000 chars}   
      and Assigned(MasterList) and (DocS <> '') then
...
if (Sym = IDSy) and (S <> '') and Assigned(MasterList) then   
...
while not (Sy in[SelectEndSy, InputSy, PSy, EofSy]+TableTermSet) do
  ...
  case Sy of
    OptionSy, OptionEndSy:
      ...
      WS := WideTrim(WS);
      if InOption then
        Select.AddStr(WS, Value, Selected, CodePage);
      WS := '';
      Value := '';
      Selected := False;
    ...
    TextSy: if InOption then
              if (Length(LCToken.S) = 1) and (LCToken.S[1] = NbSp) then
                WS := WS + ' '
              else WS := WS+LCToken.S;
    end;
  Next;
  end;
if InOption then
...
if (Sender is ThtmlViewer) and (CompareText(HttpEq, 'content-type') = 0) then
  ...
  if TranslateCharset(Content, CharSet) then
    PropStack.Last.AssignCharset(Charset)
  else if Pos('utf-8', Content) > 0 then
...
I := Pos(#0, DocS);
if I > 0 then
  SetLength(DocS, I) {not a valid file}
else
  begin
...
try
  case ALoadStyle of
    ...
    lsBuffer:
      begin
      SetLength(DocS, BuffSize);
      Move(ABuffer^, DocS[1], BuffSize);
      end;
    end;
