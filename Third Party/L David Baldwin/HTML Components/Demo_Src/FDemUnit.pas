...
uses
  ...
  FontDlg, HTMLAbt, Submit, ImgForm, MPlayer, Readhtml, FramBrwz, ComCtrls;
...
with Canvas do
  ...
  if FrameViewer.ActiveViewer <> Nil then
    begin
    TextOut(50, H-5, FrameViewer.ActiveViewer.DocumentTitle);
    SetTextAlign(Handle, TA_Bottom or TA_Right);
    TextOut(W-50, H-5, FrameViewer.ActiveViewer.CurrentFile);
    end;
  end;
AFont.Free;
end;
...
var
  TitleStr: string;
begin
if not Timer1.Enabled and (Sender is ThtmlViewer) then
  begin
