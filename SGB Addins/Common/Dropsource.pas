...
uses
  Windows, ActiveX, ComObj, AddinClasses, DropStruct, controls, grids;
...
uses
  DropTarget, ComServ, StdCtrls, Sysutils, ComCtrls;

var
  FormatEtc : TFormatEtc;

const
  EST_WRONG_ITEMCOUNT              = 'Wrong number of items added to the drag structure.';
  EST_FORMAT_SUPPORT_INCONSISTENCY =
  'Internal error in DropSource.GetData - supported formats inconsistent';
  EST_CONTROL_NOT_SUPPORTED        = 'Control type not supported as text drag source :';
  { TJNCCDropSource }
...
begin
  ...
  if FormatEtcIn.cfFormat = CF_JNCCDATA then begin
    ...
    raise EDropSourceError.Create(EST_FORMAT_SUPPORT_INCONSISTENCY);
end;  // GetData
...
begin
  ...
  if FControl is TCustomEdit then
   with TCustomEdit(FControl) do begin
    ...
    with TStringGrid(FControl) do begin
      if Selection.Top >= 1 then begin
        ...
        for i:=Selection.Top to Selection.Bottom do begin
          for liColumn:= 0 to ColCount -1 do
    ...
    raise EDropSourceError.Create(EST_CONTROL_NOT_SUPPORTED + FControl.Classname);
end;  // GetText
...
begin
  ...
  begin
    ...
    for i := 1 to iKeyList.Header.ItemCount do
