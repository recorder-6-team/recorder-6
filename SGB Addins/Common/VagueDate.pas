  ...
  try
    case iVagueDate.DateTypeString[1] of
      ...
      'D' : if iVagueDate.DateTypeString = 'D' then
              Result:=FormatDateTime(FormatSettings.ShortDateFormat, iVagueDate.StartDate)
            else if iVagueDate.DateTypeString = 'DD' then
              Result:=FormatDateTime(FormatSettings.ShortDateFormat,iVagueDate.StartDate)+' - '+
                      FormatDateTime(FormatSettings.ShortDateFormat,iVagueDate.EndDate);
...
var
  ...
  begin
    ...
    case m of
      2 : begin
            d   := 29; {Try leap year}
            Work:= IntToStr(d) + FormatSettings.DateSeparator +
                   IntToStr(m) + FormatSettings.DateSeparator +
                   IntToStr(y);
            try
              StrToDate(Work);
            except
  ...
  begin
    DecodeDate(aDate, y, m, d);
    Work := '31' + FormatSettings.DateSeparator + '12' + FormatSettings.DateSeparator + IntToStr(y);
    Result:=StrToDate(Work);
  end; {EndOfYear}
  ...
  begin
    DecodeDate(aDate, y, m, d);
    Work  := '14' + FormatSettings.DateSeparator +
            IntToStr((m+2) mod 12) + FormatSettings.DateSeparator +
            IntToStr(y);
  ...
  begin
    ...
    if (dType <> dtUnknown) and (ErrorMsg = '') then
      try
         Result := StrToDate(IntToStr(d) + FormatSettings.DateSeparator + IntToStr(m) +
                   FormatSettings.DateSeparator + IntToStr(y))
      except
            begin
