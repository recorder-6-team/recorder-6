...
===========================================================================
Modified SGB  07/11/2006 to deal with reading and writing holes from
coordinate files
*)
...
type
  // SGB 09/06/2008
  TRectInPolygonOutcome = (rtOutside, rtContains, rtContained, rtOverlaps);
...
// addesd SGB Nov 2006
Function PolygonArea(p: Pgpc_polygon): double;
Function PointInPolygon(const x, y: double; p: Pgpc_polygon): boolean;
// June 2008
Function RectangleInPolygon(const rect: TLongRect; p: Pgpc_polygon): TRectInPolygonOutcome;
...
begin
  ...
  MALLOC(pointer(p.hole), p.num_contours * sizeof(Integer), 'hole creation'); // SGB create hole flags array
  for c := 0 to p.num_contours - 1 do
  begin
    readln(f, p.contour[c].num_vertices);
    readln(f, p.hole[c]);   // SGB read hole flag
...
begin
  ...
  begin
    ...
    writeln(f, p.hole[c]);   // SGB hole flag
    for v := 0 to p.contour[c].num_vertices - 1 do
...
//===========================================================================
// Stuff added by Stuart Ball  November 2006
//===========================================================================

Function IsClosed(contour: Tgpc_vertex_list): boolean;
begin
   Result := ((contour.Vertex[0].x = contour.Vertex[contour.num_vertices-1].x) and
         (contour.Vertex[0].y = contour.Vertex[contour.num_vertices-1].y));
end;

Function ContourArea(contour: Tgpc_vertex_list): double;
var v: integer;
begin
  Result := 0.0;
  // we need at least 3 vertices to give us an area
  if (contour.num_vertices > 2) then
  begin
    for v := 0 to contour.num_vertices - 2 do
      Result := Result +
                (contour.vertex[v].x   * contour.vertex[v+1].y) -
                (contour.vertex[v+1].x * contour.vertex[v].y);
    // if it is not a closed polygon, we need to include the closing face
    if not  IsClosed(contour) then
      Result := Result +
                (contour.vertex[0].x   * contour.vertex[contour.num_vertices-1].y) -
                (contour.vertex[contour.num_vertices-1].x * contour.vertex[0].y);

  end;
  // sign depends on the direction in which vertices were traversed
...
begin
  ...
  begin
    for c := 0 to p.num_contours - 1 do
    begin
      a := ContourArea(p.contour[c]);
      // if its a hole then take the area off the total for the polygon
      if (p.hole[c] > 0) then
        Result := Result - a
      else
        Result := Result + a;
...
Function PointInContour(const x, y: double; contour: Tgpc_vertex_list): boolean;
...
{tests for each couple of points to see if the side between them
 crosses the horizontal line going through TestPoint}
  ...
  for np := 1 to contour.num_vertices-1 do
    if ((contour.Vertex[np-1].y <= y) and
        (contour.Vertex[np].y > y)) or
         ((contour.Vertex[np-1].y > y) and
        (contour.Vertex[np].y <= y)) then
    begin
         {computes the x coordinate of the intersection}
         XIntersection := contour.Vertex[np-1].x +
             ((contour.Vertex[np].x-contour.Vertex[np-1].x)/
              (contour.Vertex[np].y-contour.Vertex[np-1].y))
              *(y - contour.Vertex[np-1].y);

         {increments appropriate counter}
         if XIntersection < x then Inc(ToTheLeftofPoint);
         if XIntersection > x then Inc(ToTheRightofPoint);
    end;

  {if the polygon is open, test for the last side}
  if not IsClosed(contour) then
  begin
    if ((contour.Vertex[contour.num_vertices-1].Y <= Y) and
        (contour.Vertex[0].Y > Y)) or
         ((contour.Vertex[contour.num_vertices-1].Y > Y) and
        (contour.Vertex[0].Y <= Y))
      then
      begin
        XIntersection := contour.Vertex[contour.num_vertices-1].X +
           ((contour.Vertex[0].X-contour.Vertex[contour.num_vertices-1].X)/
            (contour.Vertex[0].Y-contour.Vertex[contour.num_vertices-1].Y))
            *(Y-contour.Vertex[contour.num_vertices-1].Y);

        if XIntersection < X then Inc(ToTheLeftofPoint);
        if XIntersection > X then Inc(ToTheRightofPoint);
      end;
   end;
  Result := ((ToTheLeftofPoint mod 2 = 1) and (ToTheRightofPoint mod 2 = 1));
end;
...
begin
  ...
  begin
    for c := 0 to p.num_contours - 1 do
    begin
      inside := PointInContour(x, y, p.contour[c]);
      // if point is in a hole then it is not in the polygon!
      if inside then
      begin
        if (p.hole[c] > 0) then
          Result := False
...
function PointInRect(vertex: Tgpc_vertex; const rect: TLongRect): boolean;
begin
  Result := (Floor(vertex.x) >= rect.Xmin) and
            (Ceil(vertex.x) <= rect.Xmax) and
            (Floor(vertex.y) >= rect.Ymin) and
            (Ceil(vertex.y) <= rect.Ymax);
end;

Function RectangleInContour(const rect: TLongRect; contour: Tgpc_vertex_list): TRectInPolygonOutcome;
var p1, p2, p3, p4, allIn, allOut: boolean;
    i: Longint;
begin
  // test whether the four corners of the rectangle are in the polygon
  p1 := PointInContour(rect.Xmin, rect.Ymin, contour);
  p2 := PointInContour(rect.Xmax, rect.Ymin, contour);
  p3 := PointInContour(rect.Xmax, rect.Ymax, contour);
  p4 := PointInContour(rect.Xmin, rect.Ymax, contour);

  // if all four points are inside the polygon then the rectangle
  // must either be contained by the polygon or there might be an
  // invagination which crosses one of its sides and therefore
  // it overlaps
  if p1 and p2 and p3 and p4 then
  begin
    Result := rtContained;
    for i := 0 to contour.num_vertices - 1 do
      if not PointInRect(contour.vertex[i], rect) then
      begin
        Result := rtOverlaps;
        Break
      end
  end
  // if all four points of the rectangle are outside the polygon then
  // either the rectangle contains the polygon (in which case all the
  // polygon's vertices are in the rectangle) or it might be outside
  // (in which case all the polygon's vertices are outside the rect)
  // or a lobe of the polygon could cross one of its sides
  else if (not p1) and (not p2) and (not p3) and (not p4) then
  begin
    allIn := True;
    allOut := True;
    for i := 0 to contour.num_vertices - 1 do
      if PointInRect(contour.vertex[i], rect) then
        allOut := False
      else
        allIn := False;
  ...
  // if some of the rectangles points are in and some out then it must
  // overlap the polygon
  else Result := rtOverlaps;
end;
...
begin
  ...
  begin
    for c := 0 to p.num_contours - 1 do
    begin
      res := RectangleInContour(rect, p.contour[c]);
      // if rectangle is contained in a hole then it is outside the polygon!
      if (res = rtContained) and (p.hole[c] > 0) then
      begin
        res := rtOutside
      end
      // if a rectangle contains a hole then it must be entirely
      // within the polygon - i.e. contained
      else if (res = rtContains) and (p.hole[c] > 0) then
      begin
        res := rtContained
      end;
      // save the value if it is further along the sequence
      if Ord(res) > Ord(Result) then
        Result := res;
    end;
  end;
end;
